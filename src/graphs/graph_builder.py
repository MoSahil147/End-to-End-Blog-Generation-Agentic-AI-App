...
class GraphBuilder:
    ...
    def build_language_graph(self):
        ...
        ## FIX: Correct translation lambda with **state expansion
        self.graph.add_node(
            "hindi_translation",
    ...
    def setup_graph(self, usecase):
        if usecase == "topic":
            return self.build_topic_graph().compile()   # <<< FIXED
        if usecase == "language":
            return self.build_language_graph().compile() # <<< FIXED


## Below code is for LangSmith / LangGraph Studio    
llm = GroqLLM().get_llm()

## FIX: Use setup_graph properly
graph_builder = GraphBuilder(llm)
graph = graph_builder.setup_graph("topic")   # <<< FIXED